...
1. **Manuel kombinde kategori çakışması**: Kullanıcı kasıtlı olarak 15 karışık ürün ekleyip aynı kategoriden birden fazla parça (2 küpe, 2 kemer gibi) seçilebildiğini gördü — "zararımız olmaz ama profesyonel olalım" dedi. `app/manuel-kombin.tsx`'teki `toggleItem` artık aynı `slot`tan (kategoriden) yeni bir ürün seçilince öncekini otomatik bırakıyor (radyo-buton benzeri davranış, engelleyici bir hata mesajı yerine) — yeni bir kural icat edilmedi, zaten gerçek bir kombinin kategori başına tek parçası olur mantığı uygulandı. Farklı kategoriler (ör. kolye + şapka) birlikte seçilmeye devam edebiliyor, sadece aynı kategori içi çakışma (küpe+küpe, kemer+kemer) engellendi.
