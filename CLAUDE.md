...
**Bilinen güvenilirlik sınırı (2026-07-15, kullanıcı cihazda bulup bildirdi)**: Trendyol aynı linke bazen tam JSON-LD dolu bir sayfa, bazen (aynı sıklıkta) ürün verisi olmayan boş bir SPA "shell" döndürüyor — canlı ölçüldü, ~%50-75 oranında değişiyordu. Bu bir bot koruması **değil** (hiçbir engelleme/CAPTCHA metni yok, sadece Trendyol'un kendi genel ana sayfa kabuğu dönüyor) — muhtemelen load balancer arkasındaki bazı podların SSR yapmaması. Çözüm: her istekte benzersiz cache-buster query param'ı + `no-store` ile 4 deneme (ara CDN katmanlarının aynı boş yanıtı önbellekten dönmesini engellemek için) — canlı testte başarı oranını ~%25'ten ~%60'a çıkardı, ama **hâlâ %100 değil**, bu bizim kontrolümüzde olmayan bir dış site tutarsızlığı. Başarısız olursa kullanıcı butona tekrar basabilir (buton kalıcı olarak devre dışı kalmıyor), manuel doldurma her zaman yedek. `lib/productLink.ts` artık supabase-js'in jenerik "non-2xx" hata mesajı yerine Edge Function'ın response gövdesindeki asıl hata metnini gösteriyor.
