...
### Uygulanan eksik-özellik iyileştirmesi: Hesabımı Sil (Google Play zorunluluğu)
Google Play, Nisan 2023'ten beri uygulama İÇİNDEN erişilebilir bir hesap silme akışını zorunlu kılıyor — daha önce hiç yoktu. Yeni **`delete-account` Edge Function'ı** (deploy edildi): `migrate-anonymous-data` ile aynı desen — çağıranın KENDİ JWT'sinden `userId` çıkarıyor (client'tan bir id parametresi ASLA almıyor, aksi halde biri başkasının hesabını silmeyi deneyebilirdi), service-role ile sırasıyla `outfits/items/wishlist_items/packing_lists/generation_events/outfit_dislikes` (`user_id`), `partnerships` (`requester_id` veya `partner_id`), `profiles` (`id`) satırlarını siliyor, en son `auth.admin.deleteUser()` ile hesabı tamamen kaldırıyor. Her tablo AÇIKÇA silme koduna dahil edildi — bazıları zaten `auth.users`'a FK CASCADE ile bağlı (`outfit_dislikes`, `packing_lists`, `profiles`) ama `items`/`outfits`/`wishlist_items`/`partnerships`/`generation_events`'in hiçbiri gerçek bir FK'ye sahip değil (sadece RLS'te `auth.uid() = user_id` kontrolü var) — bu yüzden tek bir cascade mekanizmasına güvenmek yerine hepsi manuel silindi.

`lib/auth.ts` → `deleteAccount()`: Edge Function'ı çağırır (hata mesajı `productLink.ts`'teki gibi response gövdesinden okunuyor, jenerik "non-2xx" değil), sonra `signOut()` ile aynı temizliği yapıp (`queryClient.clear()`) yeni bir anonim oturuma geçer — kullanıcı silme sonrası boş bir uygulama durumuna düşer. `app/(tabs)/profil.tsx`'te "Çıkış Yap"ın hemen altına küçük, soluk bir "Hesabımı Sil" linki eklendi (SADECE `!isAnonymous` iken görünür, `signOut` butonuyla aynı görünürlük kuralı — anonim kullanıcının silinecek "gerçek" bir hesabı yok) — iki aşamalı `showConfirm` zinciriyle (geri alınamaz olduğu iki kez vurgulanıyor) yanlışlıkla tetiklenmeyi zorlaştırıyor.
...
### Uygulanan eksik-özellik iyileştirmesi: Kullanıcı Geri Bildirim Kanalı
Uygulama içinden hata/öneri bildirmenin hiçbir yolu yoktu, her şey kullanıcının manuel test edip bana bildirmesine bağlıydı. Yeni **`user_feedback`** tablosu (`20260727000000_add_user_feedback.sql`, append-only log, sadece kendi kayıtlarını görebiliyor/ekleyebiliyor) + `lib/hooks/useFeedback.ts` → `useSendFeedback()` + yeni **`app/geri-bildirim.tsx`** (Profil menüsüne "Geri Bildirim Gönder" olarak eklendi): 1000 karaktere kadar serbest metin, gönderince teşekkür mesajı gösterip ekranı kapatıyor. Bu bir destek/mail sistemi değil — basit bir tek-yönlü kanal, kullanıcı verdiği geri bildirimi bir daha göremiyor (SELECT policy'si sadece kendi kayıtları için, ama uygulamada bunu okuyan bir ekran yok; ben Management API ile sorgulayıp okuyacağım).

**Canlı test edildi** (geçici anon kullanıcı): kayıt eklendi, kendi kaydını görebildiği doğrulandı, ikinci bir anon kullanıcının 0 kayıt gördüğü (RLS izolasyonu) doğrulandı, test verisi silindi.

### Uygulanan eksik-özellik iyileştirmesi: Offline/Bağlantı Hatası Ele Alma
Bilinçli olarak **NetInfo gibi proaktif bir bağlantı kontrolü kütüphanesi EKLENMEDİ** — bu yeni bir native modül demek, bu da (expo-notifications örneğinde olduğu gibi) yeni bir EAS build şart koşardı. Bunun yerine REAKTİF bir çözüm: `lib/queryClient.ts`'teki `QueryClient`'a global `QueryCache`/`MutationCache` `onError`/`onSuccess` handler'ları eklendi — herhangi bir sorgu/mutation gerçek bir ağ hatasıyla (`TypeError`, "Network request failed"/"Failed to fetch" mesajları — sunucu tarafı 400/500 hataları BUNA girmiyor, onlar JSON gövdeli düzgün yanıtlar) başarısız olursa yeni `lib/stores/networkStore.ts`'teki `isOffline` bayrağı `true` oluyor, herhangi bir istek başarılı olunca otomatik `false`'a dönüyor.

- `components/ui/OfflineBanner.tsx` — bayrak `true`iken ekranın en üstünde sabit, dokunulamaz (`pointerEvents="none"`) bir "İnternet bağlantısı yok gibi görünüyor" şeridi. `app/_layout.tsx`'te `AppDialogHost`'un yanında bir kez mount edildi.
- `lib/alert.ts`'e `friendlyMessage()` — ham "TypeError: Failed to fetch" gibi teknik hata metinlerini (çağıran ~50 nokta zaten `error.message`'ı doğrudan `showAlert`'e geçiyor) TEK BİR yerden "İnternet bağlantını kontrol edip tekrar dener misin?" gibi kullanıcı dostu bir mesaja çeviriyor — her çağrı noktasını tek tek düzeltmek yerine, `showAlert`'in kendisi bunu otomatik yapıyor.

Saf JS/network-katmanı değişikliği olduğu için (yeni tablo/Edge Function yok) `tsc`/`jest`/`expo export` doğrulaması yeterli görüldü, ayrıca canlı DB testi gerekmedi. Cihazda gerçek bir bağlantı kesintisiyle (uçak modu vb.) henüz denenmedi.

### Uygulanan eksik-özellik iyileştirmesi: Sentry Crash/Hata İzleme (2026-07-20)
Kullanıcı "Sentry ne için gerekli?" diye sordu — cevap: şu ana kadar bir kullanıcının telefonunda oluşan çökme/hata bildirilmezse (Geri Bildirim Gönder'i kullanmazsa) tamamen kayıp bir bilgiydi, mağazaya çıkınca bu risk büyür. Kullanıcı kendi Sentry hesabından bir DSN verdi, `EXPO_PUBLIC_SENTRY_DSN` olarak `.env`/`.env.example`'a eklendi (DSN gizli değil — Supabase anon key gibi client'a gömülmesi amaçlanan bir değer, `notlar.txt`'ye değil doğrudan `.env`'e yazıldı).

`@sentry/react-native@~5.24.3` (`npx expo install` ile SDK 51 uyumlu sürüm) eklendi — **NATIVE bir modül**, mevcut development build'de yok. `lib/sentry.ts`: `expo-notifications`/`react-native-svg` ile AYNI lazy-require + try/catch deseni (`initSentry()`/`captureException()`) — modül yoksa sessizce no-op, eski build çökmüyor. `app/_layout.tsx`'te `initSentry()` modül yüklenir yüklenmez (component'ten ÖNCE, açılış hatalarını da yakalasın diye) çağrılıyor. `enabled: !__DEV__` — kendi geliştirme testlerimiz dashboard'u kirletmesin diye sadece production/preview build'lerde aktif.
